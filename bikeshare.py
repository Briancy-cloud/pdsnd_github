...
def get_filters():
    ...
    while city not in CITY_DATA:
        city = input('Invalid input. ' +
                    'please choose from one the following cities:\n{}\n'.format(cities)).lower()
 
    
    print('***********************start**********************') 
    display_data = input('Would you like to see the first 5 rows of the data-> Yes or NO \n').lower()
    new_city= city+'.csv'
    if(display_data == 'yes'):
        
        with open(new_city, 'r') as file:
    ...
    while month not in months and month != 'all':
        month = input("\nInvalid selection. " +
                      "Please select from one of the following options to filter by month. " +
                      "\n(All, {})\n".format(x)).lower()
    ...
    with open(new_city, 'r') as file:
            reader = csv.reader(file)
            line_count = 0
            display_data = input('Would you like to show the first 5 rows of the data-> Yes or NO \n').lower()
            if(display_data == 'yes'):
                for row in reader:
    #                 print('start')
                    if month in row[1]:
...
#         break
    ...
    day = input('Please choose from one of the following days:\nAll, {}\n'.format(x)).lower()
    while day not in days and day != 'all':
        day = input("\nInvalid selection. " +
                    "Please choose from the following options to filter by day." +
                    "\n(All, {})\n".format(x)).lower()
...
def time_stats(df):
    ...
    df['day_of_week'] = df['Start Time'].dt.weekday_name
    frequent_day = df['day_of_week'].value_counts()[0]
...
def station_stats(df):
    ...
    combined_stations = df['Combined Trip Station'].value_counts()[0]
